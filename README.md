...
1. get a mapping of GPU index (reported by slurm) to GPU UUID (reported by nvidia-smi subquery)
2. get a list of processes using GPU and which GPUs (by UUID) they're using
3. get a list of GPUs (by index) allocated by slurm
4. kill any processes using unallocated GPUs
